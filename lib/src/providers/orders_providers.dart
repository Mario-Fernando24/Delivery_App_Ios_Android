...
class OrdersProviders extends GetConnect{
  ...
  User userSesion = User.fromJson(GetStorage().read('user') ?? {});
    ...
    Future<List<Order>> findByStatusDomiciliario(String id_domiciliario,String status) async {
     ...
     return orderss;
       ...
       Future<List<Order>> findByStatusClient(String id_client,String status) async {

    
      Response response = await get(
         '$url/findByClientIdStatus/'+id_client+'/'+status,
         headers: {
           'Content-Type': 'application/json',
           'Authorization': userSesion.session_token ?? ''
         }
 ...
 Future<ResponseApi> createOrders(order) async {
  ...
  Future<ResponseApi> updateOrdersDespachada(Order order) async {
      ...
      Future<ResponseApi> updateOrdersIniciandoEntrega(Order order) async {

      Response response = await put(
          '$url/updateToIniciandoEntrega',
          order.toJson(),
          headers: {
            'Content-Type': 'application/json',
            'Authorization': userSesion.session_token ?? ''
      ...
      if(response.statusCode==401){
        ...
        return ResponseApi();
         ...
         //FINALIZANDO LA ENTREGA
         Future<ResponseApi> updateToFinalizarEntregaDelivery(Order order) async {

                Response response = await put(
                    '$url/updateToFinalizarEntregaDelivery',
                    order.toJson(),
                    headers: {
                      'Content-Type': 'application/json',
                      'Authorization': userSesion.session_token ?? ''
       ...
       Future<ResponseApi> updatePosicionDomiciliarioLatLng(Order order) async {

      Response response = await put(
          '$url/updatePosicionDomiciliarioLatLng',
          order.toJson(),
          headers: {
            'Content-Type': 'application/json',
            'Authorization': userSesion.session_token ?? ''
